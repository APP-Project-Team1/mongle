...
export default function BudgetHubScreen() {
  ...
  useEffect(() => {
    if (activeBudget) {
      setBudgetVal(activeBudget.total_budget?.toString() || '0');
    }
  }, [activeBudget]);

  useEffect(() => {
    if (items) {
      setCostItems(items.map(item => ({
        id: item.id,
        label: item.category,
        value: item.amount?.toString() || '',
        warn: false,
        hasBalance: !!item.due_date,
        balanceDue: item.due_date || '',
        balanceAmount: item.spent?.toString() || '',
        urgent: false,
      })));
    }
  }, [items]);
  ...
  const handleExportPDF = async () => {
    try {
      const budgetData = {
        totalBudget: parseInt(budgetVal) * 10000,
  ...
  const progress = Math.min(100, Math.round((usedAmount / (parseInt(budgetVal) || 1)) * 100));

  if (isBudgetsLoading || isItemsLoading) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <BudgetOptimizationModal
        ...
        costItems={costItems}
        onApplyPlan={(plan) => {
            // Logic to apply optimization via API
            Alert.alert('최적화 적용', 'AI가 제안한 최적화 방안으로 업데이트하시겠습니까?');
        }}
      />
    </SafeAreaView>
  );
...
// ── 보조 컴포넌트 ────────────────────────
function CostEditModal({ visible, items, budgetId, initialBudget, onClose }) {
  const [draft, setDraft] = useState([]);
  const [activeTab, setActiveTab] = useState('cost');
  const [openIdx, setOpenIdx] = useState(null);
  const [budgetVal, setBudgetVal] = useState(initialBudget);

  const createItemMutation = useCreateBudgetItem();
    ...
    try {
      for (const item of draft) {
        const payload = {
          ...
          due_date: item.hasBalance ? item.balanceDue : null,
          spent: item.hasBalance ? (parseInt(item.balanceAmount) || 0) : 0,
        };
                          ...
                          {item.hasBalance && (
                            <View style={{gap: 10, marginTop: 5}}>
                               <TextInput
                                ...
                                onChangeText={v => updateField(idx, 'balanceDue', v)}
                                placeholder="마감일 (YYYY-MM-DD)"
                              />
                               <TextInput
                                style={modalStyles.input}
                                value={item.balanceAmount}
                                onChangeText={v => updateField(idx, 'balanceAmount', v.replace(/\D/g, ''))}
                                placeholder="잔금 (만원)"
                                keyboardType="number-pad"
                              />
                            </View>
