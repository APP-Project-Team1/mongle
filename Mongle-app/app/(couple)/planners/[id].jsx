...
export default function PlannerDetailScreen() {
  ...
  const sendNotificationToPlanner = async ({ plannerId, plannerName, date, time, budget }) => {
    ...
    console.log('user_profiles:', selectedPlanner?.user_profiles);
    console.log('plannerUserId:', plannerUserId);
  ...
  const handleConsultSubmit = async () => {
    console.log('selectedPlanner.id:', selectedPlanner?.id);
    setSubmitting(true);
    try {
      // 1. 발신자 ID 확보
      ...
      // 2. 수신자 ID 확보 - user_profiles에서 직접 조회
      const { data: userProfile, error: profileError } = await supabase
        .from('user_profiles')
        .select('id')
        .eq('planner_id', selectedPlanner.id) // wedding_planners.id (UUID)로 매칭
        .single();

      console.log('userProfile:', userProfile, 'error:', profileError);

      if (profileError || !userProfile) {
        throw new Error('플래너 계정 정보를 찾을 수 없습니다.');
      }

      // 3. 알림 전송
      await sendNotificationToPlanner({
        plannerId: userProfile.id,
