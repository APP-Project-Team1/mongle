...
export default function VendorDetailScreen() {
  ...
  const renderPrice = (item) => {
    ...
    const formatRange = (min, max) =>
      `${(min / 10000).toLocaleString()}~${(max / 10000).toLocaleString()}만원`;
    ...
    try {
      switch (category) {
          ...
          if (pricing.price_min && pricing.price_max) {
            return (
              <Text style={styles.priceText}>
                {formatRange(pricing.price_min, pricing.price_max)}
              </Text>
            );
        ...
        case 'dress':
          const dMin =
            pricing.rental_price_min || pricing.price_min || pricing.meal_price_per_person;
        ...
        case 'makeup':
          const mPrice =
            pricing.bride_hair_makeup_price || pricing.price_min || pricing.meal_price_per_person;
          if (mPrice) {
        ...
        default:
          if (pricing.meal_price_per_person && pricing.rental_fee_base) {
            ...
            return (
              <Text style={styles.priceText}>{formatMeal(pricing.meal_price_per_person)}</Text>
            );
  ...
  const renderBottomTab = () => (
    <View style={tabStyles.container}>
      ...
      <TouchableOpacity
        style={tabStyles.tabItem}
        onPress={() => router.replace('/(couple)/budget')}
      >
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.detailContent} showsVerticalScrollIndicator={false}>
        <Image
          source={{
            uri: selectedVendor.content?.thumbnail_url?.startsWith('//')
              ? `https:${selectedVendor.content.thumbnail_url}`
              : selectedVendor.content?.thumbnail_url ||
                `https://picsum.photos/seed/${selectedVendor.basic_info.vendor_id}/800/500`,
          }}
        ...
        <View style={styles.detailBody}>
          <View style={styles.detailTitleRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.detailCategoryText}>
                {CATEGORIES.find((c) => c.id === selectedVendor.basic_info.category)?.label}
              </Text>
              <Text style={styles.detailName}>{selectedVendor.basic_info.name}</Text>
            </View>
            {(selectedVendor.basic_info.rating ||
              selectedVendor.content?.rating_info?.rating_avg) && (
          ...
          <View style={styles.tagRow}>
            {selectedVendor.content?.tags &&
              selectedVendor.content.tags.map((s, idx) => (
          ...
          <View style={styles.infoSection}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>
                {selectedVendor.basic_info.address || selectedVendor.basic_info.region}
              </Text>
            </View>
            {selectedVendor.basic_info.phone && (
              <TouchableOpacity
                style={styles.infoRow}
                onPress={() => Linking.openURL(`tel:${selectedVendor.basic_info.phone}`)}
              >
            ...
            {selectedVendor.basic_info.website_url && (
              <TouchableOpacity
                style={styles.infoRow}
                onPress={() => Linking.openURL(selectedVendor.basic_info.website_url)}
              >
                <Ionicons name="globe-outline" size={20} color="#8a7870" />
                <Text style={[styles.infoText, { color: '#007AFF' }]} numberOfLines={1}>
                  {selectedVendor.basic_info.website_url}
                </Text>
              </TouchableOpacity>
            )}
          ...
          <View style={styles.priceSection}>
            ...
            <Text style={styles.priceSubText}>
              *{' '}
              {selectedVendor.pricing?.price_note ||
                '실제 견적은 상담 내용에 따라 다를 수 있습니다.'}
            </Text>
          </View>
          ...
          <View style={styles.detailInfoSection}>
            ...
            {renderDetailRow(
              '홀 개수',
              selectedVendor.details?.hall_count ? `${selectedVendor.details.hall_count}개` : null,
            )}
            {renderDetailRow(
              '예식 시간',
              selectedVendor.details?.ceremony_time_minutes
                ? `${selectedVendor.details.ceremony_time_minutes}분`
                : null,
            )}
            ...
            {renderDetailRow(
              '피팅 가능',
              selectedVendor.details?.fittings_available ? '가능' : null,
            )}
          </View>
          ...
          {renderBulletList(
            '긍정적인 포인트',
            selectedVendor.content?.review_summary_positive,
            '#4CAF50',
          )}
          {renderBulletList(
            '주의할 포인트',
            selectedVendor.content?.review_summary_negative,
            '#F44336',
          )}
        </View>
