...
export default function VendorsScreen() {
  ...
  const renderPrice = (item, isDetail = false) => {
    ...
    if (!pricing)
      return <Text style={isDetail ? styles.priceText : styles.vendorPrice}>가격 정보 없음</Text>;
      ...
      switch (category) {
        ...
        case 'package':
          if (pricing.price_min && pricing.price_max) {
            ...
            return (
              <Text style={isDetail ? styles.priceText : styles.vendorPrice}>
                {formatMinOnly(pricing.price_min)}
              </Text>
            );
          }
          break;
        ...
        default:
          if (pricing.meal_price_per_person && pricing.rental_fee_base) {
            return (
              <View style={styles.vendorPriceContainer}>
                <Text style={styles.vendorPriceSmall}>
                  {formatMeal(pricing.meal_price_per_person)}
                </Text>
            ...
            return (
              <Text style={styles.vendorPrice}>{formatMeal(pricing.meal_price_per_person)}</Text>
            );
      ...
      <FlatList
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            isSelected && styles.filterValueChipActive,
                          ]}
                        ...
                        >
                          <Text
                            style={[
                              ...
                              isSelected && styles.filterValueTextActive,
                            ]}
                          >
                            {val}
                          </Text>
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() =>
              router.push(`/vendors/${item.basic_info.vendor_id}?category=${activeCategory}`)
            }
            activeOpacity={0.8}
          >
            <Image
              source={{
                uri: item.content?.thumbnail_url?.startsWith('//')
                  ? `https:${item.content.thumbnail_url}`
                  : item.content?.thumbnail_url ||
                    `https://picsum.photos/seed/${item.basic_info.vendor_id}/300/200`,
              }}
            ...
            <View style={styles.cardInfo}>
              ...
              <View style={styles.vendorFooter}>
                <Text style={styles.vendorLocation}>
                  {item.basic_info.district || item.basic_info.region}
                </Text>
                {renderPrice(item)}
              </View>
            </View>
